...
use ethers_core::utils::{keccak256, hex::FromHex};
use halo2_base::{
    gates::builder::{
...
pub fn run_builder<T: DeserializeOwned>(
    ...
    let name = &cli.name;
    let input_path = PathBuf::from("my_proofs")
...
pub fn run_cli<P: PreCircuit>(precircuit: P, cli: Cli) {
    ...
    let data_path = cli.data_path.unwrap_or_else(|| PathBuf::from("my_proofs"));
    ...
    match cli.command {
        ...
        SnarkCmd::Keygen => {
            let pk_path = config_path.join(PathBuf::from(format!("merkleproof.pk")));
            if pk_path.exists() {
                fs::remove_file(&pk_path).unwrap();
            }
            let pinning_path = config_path.join(PathBuf::from(format!("merkle_proof.json")));
            let pk = precircuit.create_pk(&params, &pk_path, pinning_path);
            println!("Proving key written to: {pk_path:?}");

            let vk_path = config_path.join(PathBuf::from(format!("merkleproof.vk")));
        ...
        SnarkCmd::VerifyScore => {
            let vk_path = config_path.join(PathBuf::from(format!("merkleproof.vk")));
            let circuit = precircuit.create_circuit(CircuitBuilderStage::Keygen, None, &params);
            let vk = custom_read_vk(vk_path, &circuit);
            let snark_path = data_path.join(PathBuf::from(format!("{name}.snark")));
            let json_path = data_path.join(PathBuf::from(format!("{name}_proof.json")));
            let json: Value = serde_json::from_reader(
                File::open(&json_path)
                    .unwrap_or_else(|e| panic!("Input file not found at {json_path:?}. {e:?}")),
            )
            .expect("Input file should be a valid JSON file");
            let snark = read_snark(&snark_path)
                .unwrap_or_else(|e| panic!("Snark not found at {snark_path:?}. {e:?}"));
            ...
            >(verifier_params, &vk, strategy, &[&[instance]], &mut transcript)
            .unwrap();
            let mut rupper = instance[0].to_bytes()[0..16].to_vec();
            let mut rlower = instance[1].to_bytes()[0..16].to_vec();
            rlower.append(&mut rupper);
            let mut hupper = instance[2].to_bytes()[0..16].to_vec();
            let mut hlower = instance[3].to_bytes()[0..16].to_vec();
            hlower.append(&mut hupper);
            let mut fupper = instance[4].to_bytes()[0..16].to_vec();
            let mut flower = instance[5].to_bytes()[0..16].to_vec();
            flower.append(&mut fupper);
            print!("ROOT: 0x");
            for i in 0..32 {
                print!("{:02x}", rlower[31 - i]);
            }
            println!();
            print!("HEADERS: 0x");
            for i in 0..32 {
                print!("{:02x}", hlower[31 - i]);
            }
            println!();
            print!("FIELDS: 0x");
            for i in 0..32 {
                print!("{:02x}", flower[31 - i]);
            }
            println!();
            let root_bytes: Vec<u8> = Vec::from_hex(serde_json::from_value::<String>(json["root"].clone()).unwrap()).unwrap();
            let hconcat: String = serde_json::from_value(json["header"]["concat"].clone()).unwrap();
            let hlen: Vec<usize> = serde_json::from_value(json["header"]["lens"].clone()).unwrap();
            let fconcat: String = serde_json::from_value(json["fields"]["concat"].clone()).unwrap();
            let flen: Vec<usize> = serde_json::from_value(json["fields"]["lens"].clone()).unwrap();
            let mut hsums = vec![0];
            let mut header = vec![];
            let mut fields = vec![];
            let mut hlbytes = vec![];
            for i in 0..hlen.len() {
                hsums.push(hsums[i] + hlen[i]);
                header.push(hconcat[hsums[i]..hsums[i + 1]].to_string());
                hlbytes.push((hlen[i] / 256) as u8);
                hlbytes.push((hlen[i] % 256) as u8);
            }
            let mut fsums = vec![0];
            let mut flbytes = vec![];
            for i in 0..flen.len() {
                fsums.push(fsums[i] + flen[i]);
                fields.push(fconcat[fsums[i]..fsums[i + 1]].to_string());
                flbytes.push((flen[i] / 256) as u8);
                flbytes.push((flen[i] % 256) as u8);
            }
            for i in 0..hlen.len() {
                print!("{:?}", header[i]);
            ...
            let mut hchash = keccak256(hconcat).to_vec();
            let mut hlhash = keccak256(hlbytes).to_vec();
            let mut fchash = keccak256(fconcat).to_vec();
            let mut flhash = keccak256(flbytes).to_vec();
            hchash.append(&mut hlhash);
            fchash.append(&mut flhash);
            let hhash = keccak256(hchash);
            let fhash = keccak256(fchash);
            for i in 0..32 {
                assert_eq!(hhash[i], hlower[31 - i]);
                assert_eq!(fhash[i], flower[31 - i]);
                assert_eq!(root_bytes[i], rlower[31 - i]);
            }
            println!("Snark verified successfully!");
        }
    }
